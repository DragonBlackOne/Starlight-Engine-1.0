...
void main() {
    ...
    float shadow_val = calculate_shadow(v_frag_pos, N);
    ...
    // G-Buffer Outputs (Phase 5)
    gAlbedo = vec4(albedo, u_base_color.a);
    gNormal = N;
    gPosition = v_frag_pos;

    // HDR tone mapping and gamma correction for legacy color output
    vec3 out_hdr = color / (color + vec3(1.0));
    out_hdr = pow(out_hdr, vec3(1.0/2.2)); 

    outColor = vec4(out_hdr, u_base_color.a);
}
