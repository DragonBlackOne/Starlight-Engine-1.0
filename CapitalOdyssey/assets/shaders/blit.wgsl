...
/*
const ssao_samples = array<vec3<f32>, 16>(
...
fn compute_ssao(uv: vec2<f32>) -> f32 {
    let dummy_depth = textureSample(t_depth, s_diffuse, uv);
    let dummy_mat = pp.inv_view_proj[0][0];
    return dummy_depth * 0.0 + dummy_mat * 0.0;
}
