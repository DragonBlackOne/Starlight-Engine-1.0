...
// Robust Screen-Space Derivative Cotangent Frame for Normal Mapping
...
float CalculateShadow(vec3 fragPosWorld, vec3 normal) {
    ...
    float shadow = 0.0;
    ivec2 sSize = textureSize(shadowMap, 0).xy;
    if (sSize.x <= 0 || sSize.y <= 0) return 0.0;
    vec2 texelSize = 1.0 / vec2(sSize);
    
    // Sloped depth bias based on light direction
    float bias = max(0.035 * (1.0 - dot(normal, -lightDir)), 0.0035);
    if (layer == 3) {
        bias *= 0.1;
    }

    // 16-Tap Poisson Disk PCF Soft Filter with cascade-adaptive filter radius
    float filterRadius = (layer == 0) ? 1.5 : ((layer == 1) ? 2.2 : 3.0);
    for(int i = 0; i < 16; ++i) {
...
void main() {
    ...
    float matMetallic = clamp(metallic, 0.0, 1.0);
    float matRoughness = clamp(roughness, 0.04, 1.0);
    float matAO = clamp(ao, 0.0, 1.0);

    if (useMetallicRoughnessMap != 0) {
        vec4 mr = texture(metallicRoughnessMap, TexCoords);
        matMetallic = clamp(mr.b * metallic, 0.0, 1.0);
        matRoughness = clamp(mr.g * roughness, 0.04, 1.0);
        matAO = clamp(mr.r * ao, 0.0, 1.0);
    }

    // 2. Normal Mapping (via robust screen-space derivative cotangent frame)
    vec3 N = normalize(Normal);
    if (useNormalMap != 0) {
        vec3 mapN = texture(normalMap, TexCoords).xyz * 2.0 - 1.0;
        mat3 TBN = CotangentFrame(N, WorldPos, TexCoords);
        vec3 transformedN = TBN * mapN;
        if (dot(transformedN, transformedN) > 1e-4) {
            N = normalize(transformedN);
        }
    }
    ...
    vec3 dirRadiance = vec3(1.4, 1.35, 1.25) * 1.8;
    ...
    if (isSkin != 0) {
        ...
        float wrapNdotL = max(dot(N, Ldir) * 0.5 + 0.5, 0.0);
        SSS = (sssDot * 0.45 + wrapNdotL * 0.25) * sssSubColor * dirRadiance * (1.0 - matMetallic);
    }

    // 6. Ambient Lighting (Split-Sum IBL with Physical Hemispherical Fallback)
    vec3 ambient = vec3(0.0);
    vec3 skyColor = vec3(0.58, 0.68, 0.85);
    vec3 groundColor = vec3(0.35, 0.28, 0.22);
    float upFactor = N.y * 0.5 + 0.5;
    vec3 hemiAmbient = mix(groundColor, skyColor, upFactor) * 0.55 * matAlbedo * matAO;
    ...
    if (useIBL != 0) {
        ...
        ambient = max((kD_IBL * diffuseIBL + specularIBL) * matAO, hemiAmbient);
    } else {
        ambient = hemiAmbient;
    }

    // 7. PS2 Capcom Cinematic Rim-Lighting
    float rimDot = 1.0 - max(dot(N, V), 0.0);
    float rim = pow(clamp(rimDot, 0.0, 1.0), 3.5);
    vec3 rimColor = (isSkin != 0) ? vec3(1.0, 0.78, 0.55) : vec3(1.0, 0.92, 0.75);
    vec3 rimLight = rim * rimColor * 0.45 * (1.0 - matMetallic * 0.4);

    // 8. Final Combined Lighting with Deep Soft Shadows & Rim Light
    vec3 color = ambient + Lo + (1.0 - shadow * 0.82) * LoDir + SSS + rimLight;

    // 9. Atmospheric Distance Fog
