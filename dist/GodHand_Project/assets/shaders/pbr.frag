...
void main() {
    ...
    vec3 baseSkyAmbient = vec3(0.45, 0.42, 0.40) * matAlbedo;
    vec3 ambient = (kD_IBL * (diffuseIBL + baseSkyAmbient) + specularIBL) * matAO;

    // 7. Final Combined Lighting
    vec3 color = ambient + Lo + max(0.25, (1.0 - shadow)) * LoDir + SSS;
