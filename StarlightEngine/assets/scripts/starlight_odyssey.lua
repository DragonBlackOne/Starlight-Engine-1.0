-- ==========================================
-- STARLIGHT ODYSSEY: CYBER SNAKE 2D
-- Uses ENGINE Renderer2D (batched quads)
-- ==========================================
...
function UpdateSnake2D(dt)
    ...
    if InputCooldown <= 0 then
        if Engine.is_down("W") or Engine.is_down("Up") then
            if Snake.dir.y ~= 1 then Snake.nextDir = {x=0, y=-1}; InputCooldown = 0.03; Snake.started = true end
        end
        if Engine.is_down("S") or Engine.is_down("Down") then
            if Snake.dir.y ~= -1 then Snake.nextDir = {x=0, y=1}; InputCooldown = 0.03; Snake.started = true end
        end
        if Engine.is_down("A") or Engine.is_down("Left") then
            if Snake.dir.x ~= 1 then Snake.nextDir = {x=-1, y=0}; InputCooldown = 0.03; Snake.started = true end
        end
        if Engine.is_down("D") or Engine.is_down("Right") then
            if Snake.dir.x ~= -1 then Snake.nextDir = {x=1, y=0}; InputCooldown = 0.03; Snake.started = true end
        end
    end
...
-- ==========================================
-- DRAWING WITH ENGINE Renderer2D
-- ==========================================
function DrawGrid()
    -- Border (bright outline)
    gfx.draw_rect(OFFSET_X - 3, OFFSET_Y - 3, COLS * CELL + 6, ROWS * CELL + 6, 0.15, 0.25, 0.15)
    -- Background
    gfx.draw_rect(OFFSET_X, OFFSET_Y, COLS * CELL, ROWS * CELL, 0.03, 0.05, 0.03)
    
    -- Grid dots (subtle)
    for c = 1, COLS - 1 do
        for r = 1, ROWS - 1 do
            local px = OFFSET_X + c * CELL
            local py = OFFSET_Y + r * CELL
...
function DrawSnake()
    for i, seg in ipairs(Snake.body) do
        local px = OFFSET_X + seg.x * CELL + 1
        local py = OFFSET_Y + seg.y * CELL + 1
        local s = CELL - 2
        
        if i == 1 then
            -- Head
            if Snake.alive then
                gfx.draw_rect(px, py, s, s, 0.2, 1.0, 0.4)
                -- Eye effect
                gfx.draw_rect(px + 4, py + 4, s - 8, s - 8, 0.3, 1.0, 0.6)
            else
            ...
            gfx.draw_rect(px, py, s, s, 0.05, 0.7 * fade, 0.2 * fade)
            -- Inner lighter
            gfx.draw_rect(px + 2, py + 2, s - 4, s - 4, 0.08, 0.8 * fade, 0.25 * fade)
        end
    end
end

function DrawFood()
    local px = OFFSET_X + Snake.food.x * CELL
    local py = OFFSET_Y + Snake.food.y * CELL
    local pulse = math.sin(State.Time * 6) * 0.12 + 0.88
    local pad = math.floor((1.0 - pulse) * CELL * 0.3)
    -- Outer glow
    gfx.draw_rect(px + pad - 2, py + pad - 2, CELL - pad*2 + 4, CELL - pad*2 + 4, 0.4, 0.05, 0.05)
    -- Main
    gfx.draw_rect(px + pad, py + pad, CELL - pad*2, CELL - pad*2, 1.0, 0.15, 0.15)
    -- Inner shine
    gfx.draw_rect(px + pad + 4, py + pad + 4, CELL - pad*2 - 8, CELL - pad*2 - 8, 1.0, 0.5, 0.3)
end
...
function OnUpdate(dt)
    ...
    if Mode == "HUB" then
        if Engine.is_down("1") then
            Mode = "SNAKE"
            ResetSnake()
        end
    elseif Mode == "SNAKE" then
        UpdateSnake2D(dt)
        if not Snake.alive and Engine.is_down("R") then
            ResetSnake()
        end
        if Engine.is_down("Escape") then
            Mode = "HUB"
        end
    end
end
...
function OnRenderUI()
    ...
    if Mode == "HUB" then
        -- Full background via engine Renderer2D
        gfx.draw_rect(0, 0, 1280, 720, 0.02, 0.03, 0.05)
        
        -- Title bar
        gfx.draw_rect(350, 160, 580, 4, 0.2, 1.0, 0.4)
        gfx.draw_rect(350, 270, 580, 2, 0.1, 0.3, 0.15)
        ...
        -- Text overlays (ImGui for text since Renderer2D doesn't do text)
        imgui.text(440, 180, 0.2, 1.0, 0.4, "STARLIGHT ENGINE")
        imgui.text(460, 215, 0.4, 0.5, 0.6, "Odyssey Tech Demo v11")
        imgui.text(460, 305, 0.2, 1.0, 0.4, "[1]  CYBER SNAKE 2D")
        imgui.text(460, 360, 0.2, 0.2, 0.3, "More games coming soon...")
        
        imgui.text(440, 460, 0.12, 0.18, 0.22, "Renderer2D Batched Quads")
        imgui.text(440, 480, 0.12, 0.18, 0.22, "PBR + CSM Shadows + Lua Scripting")
        imgui.text(440, 500, 0.12, 0.18, 0.22, "Jolt Physics + Hot-Reload + ECS")
        
    elseif Mode == "SNAKE" then
        -- Full screen background via engine
        gfx.draw_rect(0, 0, 1280, 720, 0.01, 0.02, 0.01)
        
        -- Draw game using Engine Renderer2D
        DrawGrid()
        DrawFood()
        DrawSnake()
        ...
        gfx.draw_rect(rightX - 5, OFFSET_Y - 5, 210, 300, 0.04, 0.06, 0.04)
        gfx.draw_rect(rightX - 3, OFFSET_Y - 3, 206, 296, 0.02, 0.03, 0.02)
        
        -- HUD text
        imgui.text(OFFSET_X, 20, 0.2, 1.0, 0.4, "CYBER SNAKE 2D")
        imgui.text(OFFSET_X, 48, 0.3, 0.4, 0.35, "Rendered with Engine.Renderer2D")
        ...
        imgui.text(rightX, OFFSET_Y + 155, 0.3, 0.3, 0.4, "Controls:")
        imgui.text(rightX, OFFSET_Y + 175, 0.4, 0.7, 0.4, "WASD / Arrows")
        imgui.text(rightX, OFFSET_Y + 195, 0.3, 0.3, 0.4, "ESC quit | R restart")
        ...
        if not Snake.alive then
            ...
            imgui.text(OFFSET_X + 250, OFFSET_Y + 290, 0.4, 1, 0.5, "Press [R] to play again")
        end
    end
